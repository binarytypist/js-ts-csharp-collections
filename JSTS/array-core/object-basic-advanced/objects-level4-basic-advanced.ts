...
const teamA: TeamUser[] = [
  { id: 1, name: "A" },
  { id: 2, name: "B" },
  { id: 3, name: "C" }
];

const teamB: TeamUser[] = [
  { id: 3, name: "C2" },
  { id: 1, name: "A2" },
  { id: 2, name: "B2" }
];
...
function compareObjectsById(arr1: TeamUser[], arr2: TeamUser[]): boolean {
...
const groupedByDepartment = employees.reduce<Record<string, Employee[]>>((acc, emp) => {
...
const countByDepartment = employees.reduce<Record<string, number>>((acc, emp) => {
...
const transformedUsers: ViewUser[] = apiResponse.items
  .map((item) => ({
    id: item.user_id,
...
const transformedSummary = {
  ...
  byPrimarySkill: transformedUsers.reduce<Record<string, number>>((acc, u) => {
