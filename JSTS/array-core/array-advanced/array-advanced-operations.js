...
const revenueByUser = sales.reduce((acc, s) => {
...
const uniqueItems = [...new Set(allItems)].toSorted();
...
const frequency = allItems.reduce((acc, item) => {
  acc[item] = (acc[item] || 0) + 1;
  return acc;
}, {});
console.log("15 frequency map:", frequency);

const top3Amounts = sales.map((s) => s.amount).toSorted((a, b) => b - a).slice(0, 3);
