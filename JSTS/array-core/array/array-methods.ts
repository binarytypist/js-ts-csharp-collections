...
check("map returns new array", (names as unknown) !== (users as unknown));
check("map result is an array", Array.isArray(names) && names.length === 4);

// Spread with objects is a shallow copy: array is new, nested objects are shared.
section("shallow copy with spread");
type SimplePerson = { id: number; name: string };
...
const lastAgeOver25 = [...users].reverse().find((user) => user.age > 25);
const lastIndexAgeOver25 = users.length - 1 - [...users].reverse().findIndex((user) => user.age > 25);
...
const usersById = users.reduce<Record<number, User>>((result, user) => {
  result[user.id] = user;
  return result;
}, {});
...
// slice copies without mutation; splice mutates; immutable splice is simulated here.
...
const immutableSplice = toSplicedNumbers.slice(0, 1).concat(toSplicedNumbers.slice(3));
console.log("immutable splice:", immutableSplice);
console.log("original after immutable splice:", toSplicedNumbers);
check("slice", sameArray(sliced, [20, 30, 40]));
check("splice", sameArray(spliceNumbers, [10, 40]));
check("immutable splice result", sameArray(immutableSplice, [10, 40]));
check("immutable splice preserves original", sameArray(toSplicedNumbers, [10, 20, 30, 40]));
...
// reverse mutates; immutable reverse is simulated here.
section("reverse / immutable reverse");
const reverseNumbers = [1, 2, 3];
reverseNumbers.reverse();
const immutableReverseSource = [1, 2, 3];
const immutableReversed = immutableReverseSource.slice().reverse();
console.log("reverse mutates:", reverseNumbers);
console.log("immutable reverse:", immutableReversed);
console.log("original after immutable reverse:", immutableReverseSource);
check("reverse", sameArray(reverseNumbers, [3, 2, 1]));
check("immutable reverse", sameArray(immutableReversed, [3, 2, 1]));
check("immutable reverse preserves original", sameArray(immutableReverseSource, [1, 2, 3]));

// sort mutates in place; immutable sort is simulated here.
section("sort / immutable sort");
const sortNumbers = [30, 10, 20];
sortNumbers.sort((x, y) => x - y);
const sortedUsersByAge = users.slice().sort((x, y) => x.age - y.age);
console.log("sort mutates:", sortNumbers);
console.log("immutable sort users by age:", sortedUsersByAge.map((u) => u.name + "(" + u.age + ")"));
check("sort", sameArray(sortNumbers, [10, 20, 30]));
check("immutable sort", sameArray(sortedUsersByAge.map((u) => u.id), [4, 2, 3, 1]));

// with replaces one index immutably.
section("with");
const withNumbers = [10, 20, 30];
const withResult = withNumbers.slice(0, 1).concat([99], withNumbers.slice(2));
